...
export const ListUserManagement = () => {
  ...
  const onGetListUserAsync = async ({
    keyWord = "",
    limit = pageSize,
    page = 1,
  }) => {
  ...
  const onNavigate = (id) => {
    navigate(
      `${ROUTE_PATH.VIEW_USER.replace(`${Constants.UseParams.Id}`, "")}${id}`
    );
  };
  const listAction = (record) => {
    return (
      <Menu>
        ...
        <Menu.Item
          className="title-action"
          onClick={() => onOpenModalDelete(record.id)}
        >
    ...
    >
      ...
      <div className="flex-1 auto bg-white content-page">
        <Table dataSource={data} pagination={false}>
          ...
          />
          <Column
            title={<TitleTableCommon title="Tên người dùng" width="200px" />}
            key={"userName"}
            dataIndex={"userName"}
          />
          <Column
            title={<TitleTableCommon title="Phân quyền" width="200px" />}
            key={"role"}
            dataIndex={"role"}
            render={(value) => {
              return <div>{StatusUser(value)} </div>;
            }}
          />
          <Column
            title={<TitleTableCommon title="Họ tên" width="200px" />}
            key={"lastName"}
            dataIndex={"lastName"}
            render={(value, record) => {
              return (
          ...
          />
          <Column
            title={<TitleTableCommon title="SĐT" width="200px" />}
            key={"sdt"}
            dataIndex={"sdt"}
          />
          <Column
