...
import {
  convertTimeOnly,
  showImageCommon,
} from "../../infrastucture/utils/helper";
...
export const ListDestinationManagement = () => {
  ...
  const [districtId, setDictrictId] = useState(0);
  ...
  const onGetListLocationAsync = async ({
    ...
    if (condition) {
      const response = await api.getAllLocation(
        `${Constants.Params.search}=${keyWord.trim()}&${
          Constants.Params.limit
        }=${limit}&${Constants.Params.page}=${page}${
          idQuanHuyen != 0
            ? `&${Constants.Params.idQuanHuyen}=${idQuanHuyen}`
            : ``
        }&${Constants.Params.idDanhMuc}=${idDanhMuc}`,
        setLoading
      );
  ...
  const onChangeSearchText = (e) => {
    ...
    timeout = setTimeout(() => {
      onSearch(e.target.value, pageSize, 1, districtId, categoryId).then(
        (_) => {}
      );
  ...
  const onChangePage = async (value) => {
    setPage(value);
    await onSearch(searchText, pageSize, value, districtId, categoryId).then(
      (_) => {}
    );
  };
  ...
  const onPageSizeChanged = async (value) => {
    ...
    setSearchText("");
    setPage(1);
    await onSearch(searchText, value, page, districtId, categoryId).then(
      (_) => {}
    );
  };
  ...
  const onNavigate = (id) => {
    navigate(
      `${ROUTE_PATH.VIEW_DESTINATION.replace(
    ...
    return (
      <Menu>
        <Menu.Item
          ...
          onClick={() => onNavigate(record.idDiaDiem)}
        >
          <div className="text-base weight-600 px-1 py-0-5">Sửa</div>
        </Menu.Item>
        <Menu.Item
          className="title-action"
          onClick={() => onOpenModalDelete(record.idDiaDiem)}
        >
  ...
  return (
    ...
    >
      <div className="flex flex-col header-page">
        <Row
          className="filter-page mb-2 py-2-5"
          gutter={[10, 10]}
        ...
        >
          <Col xs={24} sm={24} lg={16}>
            <Row align={"middle"} gutter={[10, 10]}>
              ...
              <Col className="select-search" xs={24} sm={12} lg={8}>
                ...
                >
                  <Select.Option value={0} title={"Chọn TP/Huyện"}>
                    Chọn TP/Huyện
                  </Select.Option>
                  {dataDistrict &&
                    dataDistrict.length &&
              ...
              <Col className="select-search" xs={24} sm={12} lg={8}>
                <Select
                  ...
                  getPopupContainer={(trigger) => trigger.parentNode}
                >
                  <Select.Option value={1} title={"Chọn danh mục"}>
                    Chọn danh mục
                  </Select.Option>
                  {dataCategory &&
                    dataCategory.length &&
          ...
          <Col>
            <ButtonCommon
              icon={<PlusOutlined />}
              classColor="orange"
              onClick={() => navigate(ROUTE_PATH.ADD_DESTINATION)}
            >
              Thêm mới
            </ButtonCommon>
          </Col>
      ...
      <div className="flex-1 auto bg-white content-page">
        <Table dataSource={data} pagination={false} className="table-common">
          <Column
            title={"STT"}
            dataIndex="stt"
            key="stt"
          ...
          />
          <Column
            title={<TitleTableCommon title="Hình ảnh" width="100px" />}
            key={"hinhAnh"}
            dataIndex={"hinhAnh"}
            render={(val) => {
              return (
                <Image
                  src={showImageCommon(val)}
                  alt="image"
                  width={80}
                  height={48}
                />
          ...
          />
          <Column
            title={<TitleTableCommon title="Tên địa điểm" width="200px" />}
            key={"tenDiaDiem"}
            dataIndex={"tenDiaDiem"}
          />
          <Column
            title={<TitleTableCommon title="Danh mục" width="180px" />}
            key={"tenDanhMuc"}
            dataIndex={"tenDanhMuc"}
            width={"200px"}
          />
          <Column
            title={<TitleTableCommon title="Số sao trung bình" width="180px" />}
            key={"soSaoTrungBinh"}
            dataIndex={"soSaoTrungBinh"}
            render={(val, record) => {
              return <RateCommon soSao={val} luotXem={record.luotXem} />;
            }}
          />
          <Column
            title={<TitleTableCommon title="Địa chỉ" width="300px" />}
            key={"diaChi"}
            dataIndex={"diaChi"}
          />
          <Column
            title={<TitleTableCommon title="Giờ mở cửa" width={"100px"} />}
            key={"gioMoCua"}
            dataIndex={"gioMoCua"}
            render={(val) => <div>{val} </div>}
          />
          <Column
            title={<TitleTableCommon title="Giờ đóng cửa" width={"100px"} />}
            key={"gioDongCua"}
          ...
          {/* <Column
                    ...
                    /> */}
          <Column
            title={<TitleTableCommon title="Thao tác" />}
            width={"60px"}
            fixed="right"
            align="center"
