...
export const ListCategoryNewsManagement = () => {
  ...
  const onGetListCategoryAsync = async ({
    ...
    const response = await api.getAllCategoryByParentId(
      `${Constants.Params.parentId}=${
        Constants.CategoryConfig.Destination.value
  ...
  const onNavigate = (idDanhMucDiaDiem) => {
    navigate(
      `${ROUTE_PATH.VIEW_CATEGORY.replace(
    ...
    return (
      <Menu>
        <Menu.Item
          ...
          onClick={() => onNavigate(record.idDanhMucDiaDiem)}
        >
          <div className="text-base weight-600 px-1 py-0-5">Sửa</div>
        </Menu.Item>
        <Menu.Item
          className="title-action"
          onClick={() => onOpenModalDelete(record.idDanhMucDiaDiem)}
        >
  ...
  return (
    ...
    >
      <div className="flex flex-col header-page">
        <Row
          className="filter-page mb-2 py-2-5 mb-5"
        ...
        >
          ...
          <Col>
            <ButtonCommon
              icon={<PlusOutlined />}
              classColor="orange"
              onClick={() => navigate(ROUTE_PATH.ADD_CATEGORY)}
            >
              Thêm mới
            </ButtonCommon>
          </Col>
      ...
      <div className="flex-1 auto bg-white content-page">
        <Table dataSource={data} pagination={false} className="table-common">
          ...
          />
          <Column
            title={<TitleTableCommon title="Tên danh mục" />}
            key={"tenDanhMuc"}
