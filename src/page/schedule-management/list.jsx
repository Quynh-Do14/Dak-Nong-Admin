...
let timeout
export const ListScheduleManagement = () => {
    ...
    const onGetListScheduleAsync = async ({ keyWord = "", limit = pageSize, page = 1 }) => {
        const response = await api.getAllSchedule(`${Constants.Params.search}=${keyWord.trim()}&${Constants.Params.limit}=${limit}&${Constants.Params.page}= ${page}`,
            setLoading
        )
        setData(response.data.lichTrinhs);
        setPagination(response.data.pagination);
        setTotalItem(response.data.totalItems);
    }
    const onSearch = async (keyWord = "", limit = pageSize, page = 1) => {
        await onGetListScheduleAsync({ keyWord: keyWord, limit: limit, page: page })
    };
    ...
    const onNavigate = (idDanhMucDiaDiem) => {
        navigate(`${(ROUTE_PATH.VIEW_SCHEDULE).replace(`${Constants.UseParams.Id}`, "")}${idDanhMucDiaDiem}`);
    }
        ...
        <MainLayout breadcrumb={"Quản lý lịch trình"} title={"Danh sách lịch trình"} redirect={""}>
            <div className='flex flex-col header-page'>
                <Row className='filter-page mb-2 py-2-5 mb-5' justify={"space-between"} align={"middle"}>
                    ...
                    <Col>
                        <ButtonCommon icon={<PlusOutlined />} classColor="orange" onClick={() => navigate(ROUTE_PATH.ADD_SCHEDULE)} >Thêm mới</ButtonCommon>
                    </Col>
                </Row>
                ...
                >
                    ...
                    <Column
                        ...
                        render={(action, record) => (
                            ...
                            // </Space>
                            <ActionCommon
                                onClickDetail={() => onNavigate(record.idLichTrinh)}
                                onClickDelete={() => onOpenModalDelete(record.idLichTrinh)}
                            />
                            // </CommonPermission>
                        )}
                    />
            ...
            <DialogConfirmCommon
                message={"Bạn có muốn xóa lịch trình này ra khỏi hệ thống"}
                titleCancel={"Bỏ qua"}
                titleOk={"Xóa lịch trình"}
                visible={isDeleteModal}
                handleCancel={onCloseModalDelete}
                handleOk={onDeleteSchedule}
                title={"Xác nhận"}
            />
